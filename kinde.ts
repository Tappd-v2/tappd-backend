...
type Env = {
  Variables: {
    user: UserType;
    permissions: { orgCode: string, permissions: string[] };
  };
};

export const getUserWithPermissions = createMiddleware<Env>(async (c, next) => {
  try {
    ...
    const permissionsResponse = await kindeClient.getPermissions(manager);

    // Ensure orgCode is not null
    const permissions = permissionsResponse && permissionsResponse.orgCode ? {
      orgCode: permissionsResponse.orgCode,
